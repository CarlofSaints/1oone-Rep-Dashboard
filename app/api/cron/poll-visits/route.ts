...
function mapPerigeeVisit(row: Record<string, unknown>): Visit {
  ...
  if (!visitDuration && checkInTime && checkOutTime) {
    ...
    if (inParts.length >= 2 && outParts.length >= 2) {
      const diffMin = (outParts[0] * 60 + outParts[1]) - (inParts[0] * 60 + inParts[1]);
      if (diffMin > 0) {
  ...
  try {
    ...
    if (forceRun) {
      ...
      matchedSlot = schedule.slots.find(slot => {
        ...
        return Math.abs(currentMins - slotMins) <= 14;
      });
    }
    ...
    return NextResponse.json({
      ok: true, action: 'imported', imported: newVisits.length, skipped, uploadId,
    });
  } catch (err) {
