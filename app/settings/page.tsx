...
export default function SettingsPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const body: Record<string, unknown> = { endpoint: form.endpoint, enabled: form.enabled, requestBody };
  ...
  async function saveSchedule() {
    ...
    try {
      ...
      setToast({ msg: res.ok ? 'Poll schedule saved' : 'Failed to save schedule', type: res.ok ? 'success' : 'error' });
    } catch {
  ...
  return (
    <div style={{ display: 'flex' }}>
      ...
      <main style={{ flex: 1, padding: '2rem', minHeight: '100vh' }}>
        <h1 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#111827', marginBottom: '0.25rem' }}>Settings</h1>
        <p style={{ color: '#6b7280', fontSize: '0.85rem', marginBottom: '2rem' }}>Perigee API configuration and polling schedule</p>

        {/* Perigee API Config */}
        <div style={{ background: 'white', borderRadius: 12, padding: '1.5rem', border: '1px solid #e5e7eb', maxWidth: 620 }}>
          <h2 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.25rem', color: '#374151' }}>Perigee API Connection</h2>
          <p style={{ color: '#9ca3af', fontSize: '0.8rem', marginBottom: '1.25rem' }}>Endpoint and authentication</p>
          <div style={{ display: 'grid', gap: '0.75rem' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', color: '#374151', marginBottom: 4 }}>API Endpoint</label>
              <input className="input" value={form.endpoint} onChange={e => setForm({ ...form, endpoint: e.target.value })} placeholder="https://live.perigeeportal.co.za/api/visits" />
            </div>
            <div>
              ...
              <input className="input" type="password" value={form.apiKey} onChange={e => setForm({ ...form, apiKey: e.target.value })} placeholder="Leave blank to keep current token" />
            </div>
            {config?.lastPolledAt && (
        ...
        <div style={{ background: 'white', borderRadius: 12, padding: '1.5rem', border: '1px solid #e5e7eb', maxWidth: 620, marginTop: '1.5rem' }}>
          <h2 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.25rem', color: '#374151' }}>Request Body</h2>
          <p style={{ color: '#9ca3af', fontSize: '0.8rem', marginBottom: '1.25rem' }}>JSON body sent to Perigee</p>
          <textarea
            className="input"
            value={requestBody}
            ...
            spellCheck={false}
          />
          {bodyError && <div style={{ color: '#dc2626', fontSize: '0.75rem', marginTop: 4 }}>{bodyError}</div>}
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
            <button className="btn btn-outline" onClick={() => callPoll('test')} disabled={testing || !!bodyError}>{testing ? 'Testing...' : 'Test Connection'}</button>
            <button className="btn btn-primary" onClick={() => callPoll('import')} disabled={importing || !!bodyError}>{importing ? 'Importing...' : 'Import Visits'}</button>
          </div>
          {testResult && (
            <div style={{ marginTop: '1rem', padding: '0.75rem', background: testResult.ok ? '#f0fdf4' : '#fef2f2', borderRadius: 8, fontSize: '0.8rem', border: `1px solid ${testResult.ok ? '#bbf7d0' : '#fecaca'}` }}>
              {testResult.ok ? (
                <>
                  <div style={{ fontWeight: 600, color: '#166534', marginBottom: 4 }}>Connection successful — {testResult.totalRows} visits returned</div>
                  {testResult.responseKeys && testResult.responseKeys.length > 0 && (
                    <div style={{ color: '#374151', marginBottom: 4 }}><strong>Response fields:</strong> {testResult.responseKeys.join(', ')}</div>
                  )}
                ...
                <>
                  <div style={{ fontWeight: 600, color: '#991b1b', marginBottom: 4 }}>{testResult.error}</div>
                  {testResult.detail && <pre style={{ overflow: 'auto', maxHeight: 150, fontSize: '0.7rem', color: '#6b7280' }}>{testResult.detail}</pre>}
                </>
              )}
            </div>
          )}
        ...
        {/* Polling Schedule */}
        <div style={{ background: 'white', borderRadius: 12, padding: '1.5rem', border: '1px solid #e5e7eb', maxWidth: 620, marginTop: '1.5rem' }}>
          <h2 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.25rem', color: '#374151' }}>Polling Schedule</h2>
          <p style={{ color: '#9ca3af', fontSize: '0.8rem', marginBottom: '1.25rem' }}>Automated polling times (SAST). Cron runs every 30 minutes.</p>
            ...
            <div style={{ display: 'grid', gap: '0.5rem', marginBottom: '0.75rem' }}>
              {schedule.slots.map(slot => (
                <div key={slot.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem', background: '#f9fafb', borderRadius: 8 }}>
                  <input type="time" value={slot.time} onChange={e => updateSlot(slot.id, 'time', e.target.value)} style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: '0.8rem' }} />
                  <select value={slot.type} onChange={e => updateSlot(slot.id, 'type', e.target.value)} style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: '0.8rem' }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.8rem', color: '#374151', cursor: 'pointer' }}>
                    <input type="checkbox" checked={slot.enabled} onChange={e => updateSlot(slot.id, 'enabled', e.target.checked)} /> Enabled
                  </label>
                  <button onClick={() => removeSlot(slot.id)} style={{ marginLeft: 'auto', color: '#dc2626', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.8rem', padding: '4px 8px' }}>Remove</button>
                </div>
              ))}
            </div>
          )}
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.75rem' }}>
            <button className="btn btn-outline" onClick={addPollSlot}>+ Add Poll Slot</button>
            <button className="btn btn-primary" onClick={saveSchedule} disabled={savingSchedule}>{savingSchedule ? 'Saving...' : 'Save Schedule'}</button>
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 12, padding: '1.5rem', border: '1px solid #e5e7eb', maxWidth: 620, marginTop: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem' }}>
            <div>
              <h2 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.25rem', color: '#374151' }}>Cron Activity Log</h2>
              <p style={{ color: '#9ca3af', fontSize: '0.8rem' }}>Recent automated polling attempts</p>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button className="btn btn-outline" onClick={loadCronLogs} disabled={loadingLogs} style={{ fontSize: '0.75rem', padding: '4px 10px' }}>{loadingLogs ? 'Loading...' : 'Refresh'}</button>
              <button className="btn btn-primary" onClick={testCronNow} disabled={testingCron} style={{ fontSize: '0.75rem', padding: '4px 10px' }}>{testingCron ? 'Running...' : 'Test Cron Now'}</button>
            </div>
          </div>
          {cronLogs.length === 0 ? (
            <p style={{ color: '#6b7280', fontSize: '0.8rem', fontStyle: 'italic' }}>{loadingLogs ? 'Loading logs...' : 'No cron activity recorded yet.'}</p>
          ) : (
                  ...
                  {cronLogs.map((log, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #f3f4f6', background: log.error ? '#fef2f2' : log.imported && log.imported > 0 ? '#f0fdf4' : 'transparent' }}>
                      ...
                      <td style={{ padding: '6px 8px' }}>
                        <span style={{ color: log.matched ? '#16a34a' : '#9ca3af', fontWeight: 600 }}>{log.matched ? 'Yes' : 'No'}</span>
                      </td>
                      <td style={{ padding: '6px 8px' }}>{log.slotTime ? `${log.slotTime} (${log.slotType})` : '-'}</td>
                      <td style={{ padding: '6px 8px', color: log.error ? '#dc2626' : '#374151' }}>
                        {log.error ? log.error.slice(0, 60) : log.imported !== undefined ? `+${log.imported} imported, ${log.skipped ?? 0} skipped` : log.result || '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
